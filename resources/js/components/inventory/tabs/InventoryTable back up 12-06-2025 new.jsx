...
console.log("Flattened items:", flattened);
  ...
  const handleSelectProduct = (product) => {
    ...
    if (currentEditingRow) {
        const { categoryName, itemId, subItemId } = currentEditingRow;
        
        // Update all fields of the row
        handleItemChange(categoryName, itemId, 'name', product.name, true, subItemId);
        handleItemChange(categoryName, itemId, 'sku', product.hsn_code || '', true, subItemId);
        handleItemChange(categoryName, itemId, 'category', product.stock_category || '', true, subItemId);
        handleItemChange(categoryName, itemId, 'stock', product.stock_count || 0, true, subItemId);
        handleItemChange(categoryName, itemId, 'unit', product.unit || 'Piece', true, subItemId);
        handleItemChange(categoryName, itemId, 'worth', product.worth || 0, true, subItemId);
    }
                        ...
                        {expandedItems.includes(item.id) && item.subItems.map(subItem => (
                          <tr className="level-1" key={subItem.id}>
                            ...
                            <td>
                              ...
                              <button className="btn btn-sm btn-info action-btn me-1" onClick={() => setSelectedItem(subItem)}>
                              <i className="bi bi-info-circle"></i>
                              </button>
                            </td>
                          </tr>
