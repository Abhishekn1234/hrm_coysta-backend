...
const filteredProducts = (dataforSearch || []).flatMap(group => group.items || []).filter(product =>
 ...
 useEffect(() => {
  if (filteredProducts.length > 0) {
    console.log('Matching products found after filter:', filteredProducts);
  } else {
...
// };
    const handleSelectProduct = (product) => {
      setSearchTerm(product.item_name); // updates the input
      const mappedProduct = {
        id: product.id,
        name: product.item_name,               // Mapping item_name → name
      ...
      console.log("updated product search term",product.item_name );
       console.log("updated product product",product );
      setSelectedProduct(product);      // saves full product data
      setShowDropdown(false);           // hides the dropdown
    };
...
//   console.log("this is new selected product", selectedProduct);
//   const newCategoryName = selectedProduct.category ? selectedProduct.category : '';
...
//         category: selectedProduct.category || selectedProduct.stock_category || newCategoryName,
...
//   console.log("this is my selected updated data", updatedData);
//   setInventoryData(updatedData);

//   // 🧠 Save subItems to a ref so we can handle after update
...
//     pendingSubItemsRef.current = null; // clear after applying
//   }
// }, [inventoryData]); // runs whenever inventoryData updates
const handleAddProduct = (selectedProduct = null) => {
  console.log("this is new selected product", selectedProduct);
  const newCategoryName = selectedProduct.category ? selectedProduct.category : '';
