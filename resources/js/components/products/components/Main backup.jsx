...
const INITIAL_PRODUCT_STATE = {
  ...
  pricing_levels: [
    ...
    // { level: 'Wholesale', price: '', discount: 15, final_price: '' }
  ],
  // Additional fields for attributes tab
  attributes: [],
  colors: [],
  sizes: [],
  material: '',
  ...
  const handleAddProduct = async () => {
    ...
    try {
      ...
      for (const key in newProduct) {
        if (key === 'image' && newProduct.image instanceof File) {
          formData.append('image', newProduct.image);
        } else if (key === 'pricing_levels' || key === 'attributes') {
