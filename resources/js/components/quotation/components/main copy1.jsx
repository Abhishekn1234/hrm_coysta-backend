...
function Quotation() {
  ...
  const [frontPages, setFrontPages] = useState([
    { title: 'QUOTATION', content: 'Prepared for: ' }
  ]);
  ...
  const loadTemplate = (template) => {
    setFrontPages(template.frontPages);
    setBackPages(template.backPages);
 ...
 const handleCcSelect = (customer) => {
  ...
  const TemplateModal = ({ isOpen, onClose, templates, onSave, onLoad, onDelete }) => {
                        ...
                        >
                          <div>
                            ...
                            <div className="small text-muted">
                              {template.frontPages.length} front pages, 
                              {template.backPages.length} back pages
                            </div>
                          </div>
                          <div>
                            <button 
  ...
  //                         />
  //                         <div className="attachment-previews d-flex flex-wrap gap-2">
  //                           {attachmentPreviews.map((preview, index) => (
  //                               <div key={index} className="attachment-preview position-relative">
  //                                   {preview.type === 'application/pdf' ? (
  //                                   <div className="document-preview">
  //                                       <i className="fas fa-file-pdf text-danger fa-3x"></i>
  //                                       <div className="document-name">{preview.name}</div>
  //                                   </div>
  //                                   ) : (
  //                                   <img
  //                                       src={preview.url}
