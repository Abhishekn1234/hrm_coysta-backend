...
function Quotation() {
  ...
  const duplicate = quotations.find(
      (q) =>
        ...
        normalize(q.project_name) === normalize(quotationData.projectName) &&
        (!editingQuotation || q.id !== editingQuotation.id)
    );

    if (duplicate) {
      setErrorMessage(
        `A quotation for customer "${selectedCustomer?.name}" and project "${quotationData.projectName}" already exists (Quotation ID: ${duplicate.quotation_number}).`
      );
      return false;
    }
    return true;
  ...
  //     // Set success message immediately after PDF generation
  //     setSuccessMessage('PDF generated successfully!');
  //     setTimeout(() => setSuccessMessage(null), 3000); // Clear after 3 seconds

  //     // Attempt to save quotation data
  //     await saveQuotationData(quotationData, filePath);
  //   } catch (error) {
  //     console.error('Error generating or saving PDF:', error);
  //     // Only set generic error if no specific error (e.g., duplicate) was set
  //     if (!errorMessage) {
  ...
  //     console.log("this is customer data",selectedCustomer);
  //     console.log("this is quotation data",quotations);
  ...
  //     console.log("this is duplicate",duplicate);
  //     if (duplicate) {
  //       setErrorMessage(
  //         `A quotation for customer "${selectedCustomer?.name}" and project "${quotationData.projectName}" already exists (Quotation ID: ${duplicate.quotation_number}).`
  //       );
  //       throw new Error('Duplicate quotation detected.');
  //     }
  ...
  //     throw error; // Let handleGeneratePdf catch this
  //   }
  // };
  const handleGeneratePdf = async () => {
    setIsGeneratingPdf(true);
    ...
    try {
      // 1. VALIDATE FIRST (before generating PDF)
      if (!selectedCustomer) {
      ...
      // 4. GENERATE PDF (but don't download yet)
      const { filePath, pdfBlob } = await generateQuotationPdf(quotationData);

      // 5. SAVE QUOTATION DATA
      const savedQuotation = await saveQuotationData(quotationData, filePath);

      // 6. ONLY DOWNLOAD AFTER SUCCESSFUL SAVE
      const url = window.URL.createObjectURL(pdfBlob);
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `quotation_${quotationData.customerName}.pdf`);
      ...
      // Display the actual error message to user
  ...
  const handleEditQuotation = useCallback((quotation) => {
    setEditingQuotation(quotation);
  }, []);

  const handleDeleteQuotation = useCallback(async (id) => {
  ...
  const handleActionClick = useCallback((quoteId) => {
            ...
            <table className="table table-hover">
              ...
              <tbody>
                {filteredQuotations.map((quote) => {
                  // Combine parent_quotation_number and parent_chain quotation numbers
                  const parentQuotationNumbers = [
                  ...
                  return (
                    <tr key={quote.id || `quote-${Math.random()}`}>
                      ...
                      <td>{parentQuotationNumbers || '-'}</td> {/* Updated Parent Quotation column */}
                      <td>
                        <div className="d-flex gap-2">
                          {quote.status === 'rejected' && (
                            <button
                          ...
                          <a
                            ...
                            href={`http://127.0.0.1:8000/storage/${quote.pdfPath}`}
                            download={`quotation_${quote.customer_name}.pdf`}
