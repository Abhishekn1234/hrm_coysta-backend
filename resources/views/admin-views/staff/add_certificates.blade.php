    ...
    <div class="content container-fluid">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item">{{ \App\CPU\translate('Add new certificate') }}</li>
            </ol>
        </nav>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.staff.add_certificates', [$staff['id']]) }}" method="post">
                            @csrf
                                    ...
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            ...
                                            <select
                                                ...
                                                name="certificate_id" required>
                                                <option value="{{ null }}" selected disabled>Select certificate
                                                </option>
                                                @foreach ($certificate_list as $b)
                                                    <option value="{{ $b->id }}">{{ $b->certificate_name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
...
@push('script')
    ...
    <script>
        $(document).on('click', '.delete', function() {
            var id = $(this).attr("id");
            Swal.fire({
                title: "{{ \App\CPU\translate('Are_you_sure_remove_this_certificates') }}?",
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: "{{ route('admin.staff.delete_certificates') }}",
                        method: 'POST',
                        data: {
                            id: id
                        },
                        success: function(data) {
                            toastr.success(
                                '{{ \App\CPU\translate('certificates_removed_successfully') }}'
                                );
                            location.reload();
                        }
                    });
